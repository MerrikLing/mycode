    ...
    def buff(self):
        ...
        if self.strengthened:
            if self.is_container and self.ant_contained:
                if not self.ant_contained.strengthened:
                    self.ant_contained.damage *= 2
                    self.ant_contained.strengthened = True
            return
        self.damage *= 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place =self.place
        range = 0
        while(not place.is_hive and place !=None):
            if  place.bees and self.min_range <= range <= self.max_range:   #有bee
                return random_bee(place.bees) 
            place = place.entrance
            range += 1
        
        # return nearest_bee(self)
        # END PROBLEM 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            amount = self.health
            d=amount + self.damage
        else:
            d=amount
        
        for b in range(len(self.place.bees)-1,-1,-1):
            Bee.reduce_health(self.place.bees[b],d)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN PROBLEM 8
        "*** YOUR CODE HERE ***"
        return (not other.is_container) and \
            self.ant_contained == None
        # END PROBLEM 8
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        for i in range(len(self.place.bees)-1,-1,-1):
            Bee.reduce_health(self.place.bees[i],self.damage)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    # BEGIN PROBLEM 12
    implemented = True   # Change to True to view in the GUI
    def __init__(self,health=1):  #要不要？
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN PROBLEM 12
        "*** YOUR CODE HERE ***"
        if hasattr(gamestate,'hasQueen'):
                return
        if ScubaThrower.construct(gamestate):
        # if super().construct(gamestate):
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        for b in range(len(self.place.bees)-1,-1,-1):
            Bee.reduce_health(self.place.bees[b],self.damage)
        # END PROBLEM Optional 1
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN PROBLEM Optional 2
        # d=self.damage * (1-self.insects_shot*0.0625) - distance*0.25
        d=self.damage -self.insects_shot*0.0625 - distance*0.25
        if d<0:
            return 0
        return d
        # END PROBLEM Optional 2
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if self.length > 0 and gamestate.time %2 == 1:
            self.length -= 1
            return
        ...
        elif self.health > 0 and self.hasBeenScared and self.scaredturn:
            self.scaredturn -= 1
