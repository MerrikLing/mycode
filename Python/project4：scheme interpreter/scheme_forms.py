...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        e=scheme_eval(expressions.rest.first, env)#如果只用expressions.rest，会得到一个链表而不是我想要的expression中的第二个元素
        env.define(signature,e)
        return signature
        # END PROBLEM 4
...
def do_and_form(expressions, env):
    ...
    while 1:
        ...
        if is_scheme_false(value) or expressions.rest == nil:
            return value
...
def do_or_form(expressions, env):
    ...
    while 1:
        value = scheme_eval(expressions.first, env)
        if is_scheme_true(value) or expressions.rest == nil:
            return value
        expressions = expressions.rest
    # END PROBLEM 12
